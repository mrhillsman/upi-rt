resource "null_resource" "ipmi_worker" {
    ...
    provisioner "local-exec" {
        command = <<EOT
          ...
          ipmitool -I lanplus -H ${var.worker_nodes[count.index]["worker_ipmi_host"]} -p ${var.worker_nodes[count.index]["worker_ipmi_port"]} -U ${var.worker_nodes[count.index]["worker_ipmi_user"]} -P ${var.worker_nodes[count.index]["worker_ipmi_pass"]} power reset || ipmitool -I lanplus -H ${var.worker_nodes[count.index]["worker_ipmi_host"]} -p ${var.worker_nodes[count.index]["worker_ipmi_port"]} -U ${var.worker_nodes[count.index]["worker_ipmi_user"]} -P ${var.worker_nodes[count.index]["worker_ipmi_pass"]} power on;
EOT
    }
}
