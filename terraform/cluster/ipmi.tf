resource "null_resource" "ipmi_master" {
    provisioner "local-exec" {
        command = <<EOT
          ...
          ipmitool -I lanplus -H ${var.master_ipmi_host} -p ${var.master_ipmi_port} -U ${var.master_ipmi_user} -P ${var.master_ipmi_pass} power cycle || ipmitool -I lanplus -H ${var.master_ipmi_host} -U ${var.master_ipmi_user} -P ${var.master_ipmi_pass} power on;
EOT
    }
}
